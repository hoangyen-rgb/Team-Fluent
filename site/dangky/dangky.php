    ...
    <div>
        ...
        <div>
            <form action="" method="post" id="form" onsubmit="return check_form();">
                <h2>Tạo tài khoản</h2>
                <p>Để chúng tôi có thể biết bạn là ai!</p>
                
                <label>
                    <p class="input-message" id="email-message"><?php if($mail_used) {echo "Email này đã được đăng ký";} ?></p>
                    <div>
                ...
                <label>
                    <p class="input-message" id="repass-message"></p>
                    <div>
                        <svg width="15" height="19" viewBox="0 0 15 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path id="Vector" d="M12.1875 6.5625V4.6875C12.1875 2.0625 10.125 0 7.5 0C4.875 0 2.8125 2.0625 2.8125 4.6875V6.5625C1.21875 6.5625 0 7.78125 0 9.375V15.9375C0 17.5312 1.21875 18.75 2.8125 18.75H12.1875C13.7812 18.75 15 17.5312 15 15.9375V9.375C15 7.78125 13.7812 6.5625 12.1875 6.5625ZM4.6875 4.6875C4.6875 3.09375 5.90625 1.875 7.5 1.875C9.09375 1.875 10.3125 3.09375 10.3125 4.6875V6.5625H4.6875V4.6875ZM8.53125 12.6562L8.4375 12.75V14.0625C8.4375 14.625 8.0625 15 7.5 15C6.9375 15 6.5625 14.625 6.5625 14.0625V12.75C6 12.1875 5.90625 11.3437 6.46875 10.7812C7.03125 10.2187 7.875 10.125 8.4375 10.6875C9 11.1562 9.09375 12.0937 8.53125 12.6562Z" fill="#BDBDBD"/>
                        </svg>  
                        <input required class="repass" type="password" placeholder="Xác nhận mật khẩu">
                    </div>
                </label>
                <input class="submit" type="submit" value="Đăng ký" name="submit" id="submit">
                <a href="<?=$SITE_URL?>/dangnhap" class="inline-content">
...
<script>
    ...
    function check_form() {
        ...
        if (!format_sdt.test(sdt_input.value)) {
            sdt_message.innerHTML = "Số điện thoại không đúng!";
            sdt_input.value = "";
            
            submit_form = false;
        } else {
            sdt_message.innerHTML = ""; 
        }

        if (pass_input.value != repass_input.value) {
            repass_message.innerHTML = "Mật khẩu không khớp!";
            repass_input.value = "";
            
            submit_form = false;
        } else {
            repass_message.innerHTML = "";
        }
        if (submit_form) {
